...
describe('version-check', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal('TavernHelper', undefined);
  });
  ...
  it('returns no update when installation info is unavailable', async () => {
    await expect(checkPresetMemoUpdate()).resolves.toEqual({ hasUpdate: false, ok: true });
  });
  ...
  it('uses tavern extension installation info', async () => {
    ...
    await expect(checkPresetMemoUpdate()).resolves.toEqual({ hasUpdate: true, ok: true });
  });

  it('falls back to TavernHelper.getExtensionStatus', async () => {
    vi.stubGlobal('TavernHelper', {
      getExtensionStatus: vi.fn(async (id: string) => {
        expect(id).toBe(PM_EXTENSION_ID);
        return { is_up_to_date: false, current_branch_name: 'main', current_commit_hash: 'abc', remote_url: 'x' };
      }),
    });
  ...
  it('detects update when remote commit differs even if is_up_to_date is true', async () => {
    vi.stubGlobal(
      'getExtensionInstallationInfo',
      vi.fn(async () => ({
        is_up_to_date: true,
        current_branch_name: 'main',
        current_commit_hash: 'local-old',
        remote_url: 'x',
      })),
    );
    vi.stubGlobal(
