...
import { ensurePresetMemoScriptConsolidated } from './lib/script-var-migrate';
...
$(() => {
  ...
  let consolidation = { shouldRun: true } as ReturnType<typeof ensurePresetMemoScriptConsolidated>;
  try {
    consolidation = ensurePresetMemoScriptConsolidated();
  } catch (e) {
    console.warn('[预设备忘录] 重复脚本合并失败:', e);
  }

  if (!consolidation.shouldRun) {
    console.info('[预设备忘录] 检测到重复脚本，已由主实例接管，本实例跳过 UI 加载');
    return;
  }
