...
/** 草稿条目与预设条目对齐时使用的稳定 id（与 getPromptStoreId 一致） */
export function getDraftStoreId(draft: Pick<PresetPromptDraft, 'id'>): string {
...
export function applyInsertDraftToPreset(preset: Preset & Record<string, unknown>, drafts: PresetPromptDraft[]) {
  ...
  const pendingDeleteIds = new Set(
    drafts.filter(d => d.__pendingDelete).map(d => getDraftStoreId(d)).filter(Boolean),
  );
  const nextPrompts: PresetPrompt[] = [];
  const usedKeys = new Set<string>();

  finalDrafts.forEach(d => {
    const draftKey = getDraftStoreId(d);
...
export function isInsertDraftNewItem(p: PresetPromptDraft, baselineIds: Set<string>): boolean {
  ...
  const storeId = getDraftStoreId(p);
  return !storeId || !baselineIds.has(storeId);
}
...
export function getInsertDraftSaveSummary(
  ...
  final.forEach(p => {
    ...
    const storeId = getDraftStoreId(p);
    if (!storeId) return;
    const beforeEnabled = baselineEnabled.get(storeId);
...
export function createInsertDraftBaseline(prompts: PresetPromptDraft[]) {
  return {
    ids: new Set(prompts.map(p => getDraftStoreId(p)).filter(Boolean)),
    names: new Set(prompts.map(p => normalizeDraftNameKey(getDraftDisplayName(p)))),
    enabledById: new Map(
      prompts
        .map(p => [getDraftStoreId(p), p.enabled ?? true] as const)
        .filter(([id]) => !!id)
