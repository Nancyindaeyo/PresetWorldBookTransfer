...
const emit = defineEmits<{
  'expand-draft': [payload: { presetIndex: number; content: string; onApply: (c: string) => void }];
}>();
...
function openExportExpand(draft: ExportWbDraft) {
  if (draft.sourceType !== 'preset' || draft.presetIndex == null) return;
  const idx = draft.presetIndex;
  emit('expand-draft', {
    presetIndex: idx,
...
async function onJumpExportEvent(e: Event) {
  ...
  store.activeTab = 'tab-export-wb';
  if (store.exportSourceType !== 'preset') {
    store.exportSourceType = 'preset';
    store.refreshExportDrafts();
  }
  await nextTick();
...
watch(() => store.exportSourceType, () => store.refreshExportDrafts());
watch(() => store.exportMemoFolder, () => {
  if (store.exportSourceType === 'memo') store.refreshExportDrafts();
});
</script>
