<template>
  <div class="pm-varlint-list">
    <div v-if="report.macroIssues.length" class="pm-varlint-macro-issues">
      <div class="pm-varlint-macro-issues-title">其它宏 / 格式</div>
      <div
        v-for="(issue, i) in report.macroIssues"
        :key="i"
        class="pm-varlint-macro-issue"
        :class="'pm-varlint-macro-issue--' + issue.kind"
      >
        <span class="pm-varlint-macro-issue-msg">{{ issue.message }}</span>
      </div>
    </div>
    <div v-if="report.items.length === 0 && report.macroIssues.length === 0" class="pm-varlint-empty">
      当前条目未使用 setvar / getvar 或其它需检查的宏
    </div>
    <div
